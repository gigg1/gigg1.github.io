...
# Ruby features:
# NOTE: For Ruby >= 3.x / 4.x, the legacy APIs below exist but the have_func
# probes may fail against newer header layouts. Force-enable them since they
# are confirmed present in the target Ruby (4.0.6).

have_var('rb_trap_immediate', ['ruby.h', 'rubysig.h'])
have_func('rb_thread_blocking_region')
have_func('rb_thread_call_without_gvl', 'ruby/thread.h')
have_type('rb_fdset_t', 'ruby/intern.h')
add_define 'HAVE_RB_THREAD_FD_SELECT'
add_define 'HAVE_RB_WAIT_FOR_SINGLE_FD'
add_define 'HAVE_RB_THREAD_CALL_WITHOUT_GVL'
...
# Force the target architecture to match the running Ruby interpreter.
# On Apple Silicon Macs running an x86_64 Homebrew Ruby via Rosetta, the
# default clang would produce arm64 code and fail to load.
arch = RbConfig::CONFIG['arch']
if arch =~ /x86_64/
  ['CFLAGS', 'CXXFLAGS', 'LDFLAGS', 'DLDFLAGS'].each do |var|
    CONFIG[var] = "-arch x86_64 #{CONFIG[var]}" unless CONFIG[var].to_s.include?('-arch x86_64')
  end
end
